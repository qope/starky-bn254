...
const PUBLIC_INPUTS: usize = 6 * N_LIMBS + BITS_LEN + PAD;
...
mod tests {
    ...
    fn test_new_g1_exp() {
        ...
        let now = Instant::now();
        let mut pi = S::generate_public_inputs(x, x_exp, offset, exp_bits).to_vec();
        use plonky2::field::types::Field;
        pi.extend(vec![F::ZERO; PAD]);
