...
use crate::{
    ...
    input_target::{G1ExpInputTarget, G2ExpInputTarget},
    ...
    F: RichField + Extendable<D>,
{
    ...
    fn run_once(&self, pw: &PartitionWitness<F>, out_buffer: &mut GeneratedValues<F>) {
        ...
        let x_x_c0 = get_biguint(&self.input.x.x.coeffs[0].to_vec());
        let x_x_c1 = get_biguint(&self.input.x.x.coeffs[1].to_vec());
        let x_y_c0 = get_biguint(&self.input.x.y.coeffs[0].to_vec());
        let x_y_c1 = get_biguint(&self.input.x.y.coeffs[1].to_vec());
        let x_x = Fq2::new(x_x_c0.into(), x_x_c1.into());
        let x_y = Fq2::new(x_y_c0.into(), x_y_c1.into());
        let x = G2Affine::new(x_x, x_y);

        let offset_x_c0 = get_biguint(&self.input.offset.x.coeffs[0].to_vec());
        let offset_x_c1 = get_biguint(&self.input.offset.x.coeffs[1].to_vec());
        let offset_y_c0 = get_biguint(&self.input.offset.y.coeffs[0].to_vec());
        let offset_y_c1 = get_biguint(&self.input.offset.y.coeffs[1].to_vec());
    ...
    C::Hasher: AlgebraicHasher<F>,
{
    ...
    fn run_once(&self, pw: &PartitionWitness<F>, out_buffer: &mut GeneratedValues<F>) {
        ...
        let ios_native = self
            ...
            .cloned()
            .map(|input| {
                let x_x_c0 = get_biguint(&input.x.x.coeffs[0].to_vec());
                let x_x_c1 = get_biguint(&input.x.x.coeffs[1].to_vec());
                let x_y_c0 = get_biguint(&input.x.y.coeffs[0].to_vec());
                let x_y_c1 = get_biguint(&input.x.y.coeffs[1].to_vec());
                let x_x = Fq2::new(x_x_c0.into(), x_x_c1.into());
                let x_y = Fq2::new(x_y_c0.into(), x_y_c1.into());
                let x = G2Affine::new(x_x, x_y);

                let offset_x_c0 = get_biguint(&input.offset.x.coeffs[0].to_vec());
                let offset_x_c1 = get_biguint(&input.offset.x.coeffs[1].to_vec());
                let offset_y_c0 = get_biguint(&input.offset.y.coeffs[0].to_vec());
                let offset_y_c1 = get_biguint(&input.offset.y.coeffs[1].to_vec());
                let offset_x = Fq2::new(offset_x_c0.into(), offset_x_c1.into());
                let offset_y = Fq2::new(offset_y_c0.into(), offset_y_c1.into());
                let offset = G2Affine::new(offset_x, offset_y);
                let exp_val = get_biguint(&input.exp_val.to_vec());
...
mod tests {
    ...
    use crate::{
        ...
        input_target::{G1ExpInput, G1ExpInputTarget, G2ExpInputTarget},
        utils::u32_digits_to_biguint,
    };
